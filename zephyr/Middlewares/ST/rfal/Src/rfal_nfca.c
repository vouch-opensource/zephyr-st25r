...
static ReturnCode rfalNfcaPollerGetSingleCollisionResolutionStatus( void )
{
    ...
    switch( gNfca.CR.state )
    {
        ...
        case RFAL_NFCA_CR_SDD:
            ...
            EXIT_ON_BUSY( ret, rfalISO14443AGetTransceiveAnticollisionFrameStatus() );
...
ReturnCode rfalNfcaPollerGetSelectStatus( void )
{
    ...
    if( (!gNfca.SEL.isRx) )
    {
        ...
        if( gNfca.SEL.cascadeLv <= gNfca.SEL.fCascadeLv )
        {
            ...
            /*******************************************************************************/
            /* Send SEL_REQ  */
            EXIT_ON_ERR( ret, rfalTransceiveBlockingTx( (uint8_t*)&selReq, sizeof(rfalNfcaSelReq), (uint8_t*)gNfca.SEL.selRes, sizeof(rfalNfcaSelRes), &gNfca.SEL.rxLen, RFAL_TXRX_FLAGS_DEFAULT, RFAL_NFCA_FDTMIN ) );
