...
static void st25r3916comTx( const uint8_t* txBuf, uint16_t txLen, bool last, bool txOnly )
{
    ...
    if( txLen > 0U )
    {
        ...
        platformI2CTx( txBuf, txLen, last, txOnly );
            ...
            ST_MEMCPY( &comBuf[comBufIt], txBuf, MIN( txLen, (uint16_t)(ST25R3916_BUF_LEN - comBufIt) ) );    /* copy tx data to local buffer                      */
...
static void st25r3916comRx( uint8_t* rxBuf, uint16_t rxLen )
{
    ...
    #ifdef ST25R_COM_SINGLETXRX
        ST_MEMSET( &comBuf[comBufIt], 0x00, MIN( rxLen, (uint16_t)(ST25R3916_BUF_LEN - comBufIt) ) );     /* clear outgoing buffer                                  */
        platformSpiTxRx( comBuf, comBuf, MIN( (comBufIt + rxLen), ST25R3916_BUF_LEN ) );                  /* transceive as a single SPI call                        */
        ST_MEMCPY( rxBuf, &comBuf[comBufIt], MIN( rxLen, (uint16_t)(ST25R3916_BUF_LEN - comBufIt) ) );    /* copy from local buf to output buffer and skip cmd byte */
    #else
        if( rxBuf != NULL)
        {
